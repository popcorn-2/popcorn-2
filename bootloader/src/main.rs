...
fn main(image_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
   ...
   let loading_screen = LoadingScreen::new(fb.dimensions(), ::ui::pixel::Color2{
    ...
    logging::init(&mut ui, uart.deref_mut()).unwrap();
    ...
    info!("Loading popcorn2 from `{}`...", config.kernel_config.image);
    ...
    if let Ok(partition_handles) = services.locate_handle_buffer(SearchType::ByProtocol(&PartitionInfo::GUID)) {
        for partition_handle in partition_handles.iter() {
            ...
            match partition_info.gpt_partition_entry() {
                Some(gpt_entry) if {
                    let guid = gpt_entry.unique_partition_guid;
                    guid == config.kernel_config.root_disk
        ...
        /*let Some(root_partition_handle) = root_partition_handle else {
        panic!("Could not find specified root partition");
    };

    let root_partition_io = services.open_protocol_exclusive::<BlockIO>(*root_partition_handle).unwrap();*/
    ...
    for module in &modules {
        let result: Result<(),ModuleLoadError> = try {
            ...
            /*for symbol in symtab {
                debug!("{:?} - {:x?}", symbol.name.map(|str| stringtab.get_string(str)), symbol);

                if symbol.section_table_index != 0
                        && !symbol.info.is_local()
                        && let Some(idx) = symbol.name {
                    let string = stringtab.get_string(idx);

                    if string == cstr!(b"__popcorn_module_main_allocator") { testing_fn = symbol.value + u64::try_from(base).unwrap(); }
                    else if string == cstr!(b"__popcorn_module_author") {
                        let author_data = module.data_at_address(ExecutableAddress(symbol.value)).unwrap();
                        let author_data = unsafe { &*slice_from_raw_parts(author_data, symbol.size.try_into().unwrap()) };
                        author = core::str::from_utf8(author_data).map_err(|_| ModuleLoadError::InvalidAuthorMetadata)?;
                    }
                    else if string == cstr!(b"__popcorn_module_modulename") {
                        let name_data = module.data_at_address(ExecutableAddress(symbol.value)).unwrap();
                        let name_data = unsafe { &*slice_from_raw_parts(name_data, symbol.size.try_into().unwrap()) };
                        name = Some(core::str::from_utf8(name_data).map_err(|_| ModuleLoadError::InvalidNameMetadata)?);
                    }
                    else if string == cstr!(b"__popcorn_module_modulefqn") {
                        let fqn_data = module.data_at_address(ExecutableAddress(symbol.value)).unwrap();
                        let fqn_data = unsafe { &*slice_from_raw_parts(fqn_data, symbol.size.try_into().unwrap()) };
                        fqn = core::str::from_utf8(fqn_data).map_err(|_| ModuleLoadError::InvalidFqnMetadata)?;
                    }
                }
