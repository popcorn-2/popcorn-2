...
use crate::framebuffer::{FontStyle, Tui};
...
pub fn init(ui: &mut dyn FormatWrite, uart: &mut dyn Write) -> Result<(), SetLoggerError> {
	unsafe {
		LOGGER.ui = Some(mem::transmute(ui));
		LOGGER.uart = Some(mem::transmute(uart));
