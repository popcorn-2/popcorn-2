...
impl ProcInfo {
	...
	pub fn new_in(
		...
		const {
			...
			assert!(align_of::<ProcInfo>() >= align_of::<NamedHandle>(), "proc info must be aligned to page size");
		}
		...
		let page_count = (proc_info_size + handle_tab_len + string_tab_len).div_ceil(PAGE_SIZE);
		...
		let mut next_string = unsafe { named_handles.add(handles.len() + 1).cast::<u8>() };
