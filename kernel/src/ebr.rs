...
pub fn test() {
	...
	let foo = Box::into_raw(Box::new(Foo));
	let bar = Box::into_raw(Box::new(Bar));

	debug!("retire bar");
	percpu_v2!(epoch).retire_and_cleanup::<Box<_>>(bar);
	debug!("pin");
	let guard = percpu_v2!(epoch).pin();
	debug!("retire foo");
	percpu_v2!(epoch).retire_and_cleanup::<Box<_>>(foo);
	debug!("gc 1");
	gc_collect();
	drop(guard);
	debug!("gc 2");
