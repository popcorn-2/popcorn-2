...
impl HandleMap {
	...
	pub fn push(&self, handle: Arc<Handle>) -> Result<u32, Error> {
		loop {
	...
	pub fn pop(&self, val: u32) -> Result<Arc<Handle>, Error> {
		self.map.lock().remove(&val).ok_or(Error::InvalidHandle)
	}
}
...
impl Handle {
	pub fn new(server_id: ServerId, internal_id: isize, protocols: &[u128]) -> Arc<Handle> {
		Arc::new(Handle {
			server_id,
			internal_id,
			supported_protocols: HashSet::from_iter(protocols.iter().copied()),
		})
	}
