...
pub fn read_gas(gas: GenericAddress) -> Result<u64, AcpiError> {
	...
	let val = match gas.address_space {
		AddressSpace::SystemIo => {
			let mut val = 0u64;
			match access_width {
				1 => unsafe { core::arch::asm!("in al, dx", in("dx") gas.address, inout("al") val, options(nostack, nomem, preserves_flags)) },
				2 => unsafe { core::arch::asm!("in ax, dx", in("dx") gas.address, inout("ax") val, options(nostack, nomem, preserves_flags)) },
				4 => unsafe { core::arch::asm!("in eax, dx", in("dx") gas.address, inout("eax") val, options(nostack, nomem, preserves_flags)) },
