...
pub(crate) fn tsc_to_nanos() -> (u128, NonZero<u128>) {
	*TSC_MULTIPLIER.get_or_init(|| {
		...
		if max_leaf >= 0x15 {
			...
			let CpuidResult { eax: denom, ebx: num, ecx: freq, .. } = __cpuid(0x15);
			...
			if is_virtualised {
				...
				let max_leaf = __cpuid(0x40000000).eax;
				if max_leaf >= 0x40000010 {
					debug!("[TSC] Leaf 40000010h supported");
					let CpuidResult { eax: freq, .. } = __cpuid(0x40000010);
