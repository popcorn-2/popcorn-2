...
impl<T> TaggedNonNull<T> {
	...
	const MAX_BOUND_VALUE: usize = 1 << (Self::TAG_HIGH_BITS + Self::TAG_LOW_BITS);
	...
	pub const fn new(ptr: NonNull<T>, tag: usize) -> Self {
	...
	pub const fn addr_eq(&self, other: &Self) -> bool {
		core::ptr::addr_eq(self.0.as_ptr(), other.0.as_ptr())
	}
}

impl<T: ?Sized> fmt::Pointer for TaggedNonNull<T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
		...
		let mut ptr_f = *f;
		*f = f.with_options(old_options);

		f.debug_struct("TaggedNonNull")
			.field_with("addr", move |_| fmt::LowerHex::fmt(&addr, &mut ptr_f))
			.field("metadata", &meta)
			.field("tag", &tag)
			.finish()
	}
