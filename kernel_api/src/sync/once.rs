...
use core::fmt;
use core::mem::{MaybeUninit, DropGuard};
use core::ops::Deref;
...
impl const TryFrom<u8> for State {
    type Error = ();
...
impl Once {
    ...
    pub const fn new() -> Self {
        Self(AtomicU8::new(State::Uncalled.into()))
    }
    ...
    pub fn call_once<F: FnOnce()>(&self, f: F) {
        loop {
            ...
            match current {
                ...
                Err(state) if state == State::Poison.into() => panic!("poisoned `Once`"),
                Err(state) if state == State::Running.into() => {}, // Currently running, spin until state changes
                Err(state) if state == State::Called.into() => return, // Already called, return immediately
                Err(state) if state == State::Uncalled.into() => {}, // weak cas fail, try again
                _ => unreachable!()
            }
            core::hint::spin_loop();
        }

        let drop_guard = DropGuard::new(self, |this| this.0.store(State::Poison.into(), Ordering::Relaxed));

        f();

        DropGuard::dismiss(drop_guard);
